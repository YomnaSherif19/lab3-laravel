...
<h1 class="form-heading"></h1>
    ...
    <div class="main-div">
        ...
        <form id="register" method="POST" action="{{route('users.update,['id'=>$users['id']]')}}">
        @method('PUT')
            @csrf
            <div class="form-group">
                <input type="text" value="{{$users['name']}}" name="name" class="form-control" id="inputEmail" placeholder="Name">
            </div>
            <div class="form-group">
                <input type="email" value="{{$users['email']}}" name="email" class="form-control" id="inputPassword" placeholder="Email Address">
            </div>
